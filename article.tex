...
This section gives an algorithm to detect bivincular permutation in polynomial time in the case where the motif avoid $(231,213)$.
			...
			If $n_\pmotif \in \x$ there exists a match
			if and only if\\ $\bigcup_{0<\min<\max<n_\ptext}\mathbb{P}_{\pmotif,\ptext,\min,\max}(0,0)$ is true.\\
			
			If $n_\pmotif \notin \x$ there exists a match if and only if \\	$\bigcup_{0<k<n_\ptext,0<\min<\max<n_\ptext}\mathbb{P}_{\pmotif,\ptext,\min,\max}(0,k)$
			is true.			
			\end{proof}
