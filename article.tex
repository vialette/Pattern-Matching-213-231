...
This section gives an algorithm to detect bivincular permutation in polynomial time in the case where the motif avoid $(231,213)$.
			...
			\begin{itemize}
				\item If $n_\pmotif \in \x$ there exists a match if and only if there is a match with the first element of the text with every pair of min max possible :\\ $\bigcup_{0<\min<\max<n_\ptext}\mathbb{P}_{\pmotif,\ptext,\min,\max}(0,0)$ is true.
				\item If $n_\pmotif \notin \x$ there exists a match if and only if there exists a match stating ar any index in the text with every pair of min max possible : \\	$\bigcup_{0<k<n_\ptext,0<\min<\max<n_\ptext}\mathbb{P}_{\pmotif,\ptext,\min,\max}(0,k)$
							is true.	
			\end{itemize}
