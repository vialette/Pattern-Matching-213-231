...
$\pi[i:]$ starts either with the maximal element
or with the minimal element of $\pi[i:]$.
\end{corollary}
...
Given a permutation $\sigma \in \AV(213,231)$ and a suffix of its decomposition $\factor{\sigma}{i}\factor{\sigma}{i-1}\ldots\factor{\sigma}{1}$,
if $\stripe{\sigma}{\firsti{\sigma}{i}}=\ustep$
(resp. $\stripe{\sigma}{\firsti{\sigma}{i}}=\dstep$)
then the maximal (resp. minimal) element of
...
\begin{cases}
	\text{The maximal value of a subsequence}
		& \text{If $\stripe{\sigma}{\firsti{\sigma}{i}}=\ustep$ } \\
	...
	\text{The minimal value of a subsequence}
		& \text{If $\stripe{\sigma}{\firsti{\sigma}{i}}=\dstep$ } \\
	\text{ which is an occurence of $\sigma[\firsti{\sigma}{i}:]$ in $\pi[j:]$} & \\
...
\begin{cases}
		\min_{j<j'}\{\infty\} \cup \{\ptext[j'] |\text{ $j'$ such that \indent }
			& \text{If $\stripe{\sigma}{\firsti{\sigma}{i}}=\ustep$ }  \\
		\text{ $|\factor{\pmotif}{1}| \leq LIS(j,j',\ptext[j'])  $}\}& \\
		&\\
		\max_{j<j'}\{0\} \cup \{\ptext[j'] |\text{ $j'$ such that \indent }
			& \text{If $\stripe{\sigma}{\firsti{\sigma}{i}}=\dstep$ } \\
		\text{ $|\factor{\pmotif}{1}| \leq LDS(j,j',\ptext[j']) $}\}&\\
\end{cases}
$
...
\begin{cases}
	...
	\text{if $\stripe{\pmotif}{\first{\pmotif}{i}}=\ustep$}\\
	\max \{0\} \cup  \DF{\pmotif}{\ptext}{i}{j} &
	\text{if $\stripe{\pmotif}{\first{\pmotif}{i}}=\dstep$}\\
\end{cases}
$

%
%With $\AF{\pmotif}{\ptext}{i}{j}$ ($\DF{\pmotif}{\ptext}{i}{j}$) the set of element such that
%if  $\ptext[k] \in \AF{\pmotif}{\ptext}{i}{j}$ then there exists
%a match of $\pmotif[\firsti{\pmotif}{i-1}:]$ in $\ptext[k:]$ and
%the element $\first{\pmotif}{i-1}$ is matched with element $\ptext[k]$ and
%there is a match of $\factor{\pmotif}{i}$ in $\ptext[j:k-1]$ and
%the element $\first{\pmotif}{i}$ is matched with element $\ptext[j]$ and every element is inferior (superior) to the match of
%$\pmotif[\firsti{\pmotif}{i-1}:]$ in $\ptext[k:]$.
...
%\textbf{BASE :} \\
%
%For the base case, one matches the right most element of the motif. If the motif has a right bottom corner (if $n_\pmotif \in  \x$) then one must assure that the right most element of the motif is matched to the right most of the text. If the motif has a right top corner (if $n_\pmotif \in  \y$) and the right most element of the motif is maximal (minimal) then one must assure that right most element of the motif is matched to the maximal (minimal) element of the text. If there is not corner, there is no condition on the match of the right most element.
%
...
%\begin{cases}
%	true 	& \text{if $\min<\ptext[j]<\max$ }\\
%			& \text{and $\ptext[j]$ is the maximal element}\\
%	false 	& otherwise\\
%\end{cases}
%$
...
%	true 	& \text{if $\min<\ptext[j]<\max$}\\
%			& \text{and } j=n_\ptext\\
%			& \text{and $\ptext[n_\ptext]$ is the maximal element}\\
%	false 	& otherwise\\
%\end{cases}
%$
%%$\mathbb{P}_{\pbmotif,\min,\max}^{\ptext}(n_\pmotif+1,n_\ptext)= true $\\
%
%if $n_\pmotif \notin  \x$  and $n_\pmotif \in  \y$ and $\pmotif[n_\pmotif]$ is the minimal element: \\
%$
%\PM{\pbmotif}{\ptext}{\min}{\max}{n_\pmotif}{j}=
%\begin{cases}
%	true 	& \text{if $\min<\ptext[j]<\max$}\\
%			& \text{and $\ptext[j]$ is the minimal element}\\
%	false 	& otherwise\\
%\end{cases}
%$
%%$\mathbb{P}_{\pbmotif,\min,\max}^{\ptext}(n_\pmotif+1,j)= true $\\
%
%if $n_\pmotif \in  \x$  and $n_\pmotif \in  \y$ and $\pmotif[n_\pmotif]$ is the minimal element: \\
%$
%\PM{\pbmotif}{\ptext}{\min}{\max}{n_\pmotif}{j}=
%\begin{cases}
%	true 	& \text{if $\min<\ptext[j]<\max$}\\
%			& \text{and } j=n_\ptext\\
%			& \text{and $\ptext[n_\ptext]$ is the minimal element}\\
%	false 	& otherwise\\
%\end{cases}
%$
%%$\mathbb{P}_{\pbmotif,\min,\max}^{\ptext}(n_\pmotif+1,n_\ptext)= true $\\
...
%\textbf{STEP :} \\
%
%For the induction there are 8 cases to consider, depending on if $\pmotif[i]$ is ascend or descend, if elements $\pmotif[i]\pmotif[i+1]$ are underlined ($i \in \x$) and if elements $\pmotif[i]\pmotif[i+1]$ are overlined ($i \in \y$). First one must assure that $\pmotif[i]$ can be matched to $\ptext[j]$, this is done by checking that $\ptext[j]$ is in the interval $]\min,\max[$. Then one must use the induction. The element $\ptext[j]$ gives a new $\min$ if the element is ascent or a new $\max$ if the element is descent.
%If $\pmotif[i]\pmotif[i+1]$ are not underlined nor overlined, then $\pmotif[i+1]$ can be matched to any upcoming element in the text. If $\pmotif[i]\pmotif[i+1]$ are underlined ($i \in \x$) then the match of $\pmotif[i+1]$ must be $\ptext[j+1]$. If $\pmotif[i]\pmotif[i+1]$ are overlined ($i \in \y$) then ethe match of  $\pmotif[i+1]$ must be $\ptext[j]+1$.
%
%$
%\PM{\pbmotif}{\ptext}{\min}{\max}{i}{j}=
%\begin{cases}
...
%$
%
%There are 6 cases to consider to search for a match.
%Depending on if the motif has a left bottom corner ($0 \in \x$), a left top corner, and if it has a left top corner $0 \notin \y$, if the left most element is minimal or maximal. If the motif has a left bottom corner, then one must assure that the left most element of the motif is matched to the left most element of the text. If the motif has a left top corner and the left most element of the motif is minimal (maximal) then one must assure that the left most element of the motif is matched with the minimal (maximal) element of the text :
%\begin{itemize}
%	\item If $0 \in \x$ and $0 \notin \y$ there exists a match if and only if there is a match starting with left most element of the text : $ \PM{\pbmotif}{\ptext}{0}{n_{\pmotif}}{1}{1}$ is true.
%
%	\item If $0 \notin \x$ and $0 \notin \y$ there exists a match if and only if there exists a match stating at any index : 	$\bigcup_{1<k\leq n_\ptext}\PM{\pbmotif}{\ptext}{0}{n_{\pmotif}}{1}{k}$ is true.
%
%	\item If $0 \in \x$, $0 \in \y$ and the first element of the stripe of the motif is minimal there exists a match if and only if there is a match starting with the left most element  of $\ptext$, and the left most element  of $\ptext$ is the minimal element  of $\ptext$ : $\PM{\pbmotif}{\ptext}{0}{n_{\pmotif}}{1}{1}$  is true and the left most element of $\ptext$ is the minimal element of $\ptext$.
%
%	\item If $0 \notin \x$, $0 \in \y$ and the first element of the stripe of the motif is minimal there exists a match if and only if there exists a match stating at the minimal element of the text : if $j_{\min}$ is the index of the minimal element in $\ptext$ then $\PM{\pbmotif}{\ptext}{0}{n_{\pmotif}}{1}{j_{\min}}$ is true.
%
%	\item If $0 \in \x$, $0 \notin \y$  and the first element of the stripe of the motif is maximal then there exists a match if and only if there is a match starting with the left most element  of $\ptext$ and the left most element  of $\ptext$ is the maximal element  of $\ptext$: $\PM{\pbmotif}{\ptext}{0}{n_{\pmotif}}{1}{1}$ is true and the left most element of $\ptext$ is the maximal element of $\ptext$.
%
%	\item If $0 \notin \x$, $0 \notin \y$ and the first element of the stripe of the motif is maximal there exists a match if and only if there exists a match stating at the maximal element of the text : if $j_{\max}$ is the index of the maximal in $\ptext$ element then $\PM{\pbmotif}{\ptext}{0}{n_{\pmotif}}{1}{j_{\max}}$ is true.
%
%\end{itemize}
%\end{proof}
