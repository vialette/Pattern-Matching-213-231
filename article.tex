...
\end{itemize}
	...
	\section{Pattern Matching Problem With Text and Motif Avoiding (231,213)}
		...
		\textbf{Lemma  \numl{firstelement} \thenum} If a permutation is in $\Av{n}{213,231}$ 
		...
		\textbf{Corollary  \num \thenum} A permutation $\pi \in \Perm{n}$ is in $\Av{n}{213,231}$ if and only if for $1 \leq i \leq n$, 
		$\pi[i:]$ start either with the maximal element
		or the minimal element of $\pi[i:]$.\\
	
		\textbf{Corollary \numl{stripe} \thenum}  If $\pi \in \Av{n}{213,231}$ its stripe can be define such as, \\
		$\stripe{\pi}{i} = \left\{ 
  		\begin{array}{l l}
		...
		\textbf{Lemma \numl{upup} \thenum} Let $\pi \in \Av{n}{213,231}$,
		$\forall i,j \in [n]$, 
		$i<j$, 
		if $\stripe{\pi}{i}=\ustep$
...
%		$\Box$\\
		...
		\textbf{Lemma \numl{downdown} \thenum}  Let $\pi \in \Av{n}{213,231}$,
		$\forall i,j \in [n]$,
		$i<j$,
