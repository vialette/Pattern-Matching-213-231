...
\title{Pattern matching for (231,231) avoiding permutations}
...
%		and the set is the set of image of this function.\\
	...
	\section{Pattern Matching Problem With Text and Motif Avoiding (231,213)}
		...
		\textbf{Corollary \numl{stripe} \thenum}  If $p \in \C{n}$ its stripe can be define such as, \\
		$\mathcal{S}(p,i) = \left\{ 
  		\begin{array}{l l}
...
%		$\Box$\\
		...
		\textbf{Lemma \numl{downdown} \thenum}  Let $p \in \C{n}$,
		$\forall i,j \in [n]$,
		$i<j$,
		$\mathcal{S}(p,i)=\dstep$
		and $\mathcal{S}(p,j)=\dstep$
...
%		$\Box$\\
		...
		\textbf{Proposition \numl{patternMatchingStripe} \thenum} Let $\ptext, \pmotif \in \C{n}$.
