...
For example the permutation $123$ (resp. $321$) occurs in a second permutation if
the second permutation has an increasing (resp. a decreasing) subsequence of size $3$.
...
They showed that
the permutation pattern matching is fixed-parameter tractable (FPT) for by the size of $\sigma$.

A few particular cases of the permutation pattern matching problem have been attacked successfully.
The case of any increasing pattern is solvable in 
$O(n \log \log k)$ time in the RAM model \cite{Crochemore:Porat:2010},
...
(see also \cite{Bouvel:Rossin:Vialette:CPM:2007} for the longest common subsequence problem and related ones on separable permutations).
...
As a consequence, a wedge permutation can be partitioned 
...
The partition gives a bijection between the set of wedge permutations of size $n$ with elements $1,\ldots,n$
and the set of binary words of size $n-1$.
...
\begin{remark}
We can draw a horizontally line (passing thought the rightmost element) on a wedge permutations  such that the upper half contains only a  decreasing  
subsequence and the lower half contains only an increasing subsequence.
...
We consider 3 cases for the problem $\PM^{\widetilde{\sigma}}_{\ptext}(\lb,\ub,i,j)$:
\begin{itemize}
	\item If $\ptext[j] \notin [\lb,\ub]$ then:
	$$
	\PM^{\widetilde{\sigma}}_{\ptext}(\lb,\ub,i,j) = false
	$$
	...
	\PM^{\widetilde{\sigma}}_{\ptext}(\lb,\ub,i,j)=
	\begin{cases}
		\bigcup_{\ell>j} \PM^{\widetilde{\sigma}}_{\ptext}(\ptext[j]+1,\ub,i+1,\ell)
		...
		\bigcup_{\ell>j} \PM^{\widetilde{\sigma}}_{\ptext}(\ptext[j]+1,\ub,i+1,\ell)
			...
			& \text{ and $\ptext[j]=\lb$} \\
		\PM^{\widetilde{\sigma}}_{\ptext}(\ptext[j]+1,\ub,i+1,j+1)
	...
	$\PM^{\widetilde{\sigma}}_{\ptext}(\ptext[j]+1,\ub,i+1,\ell)$ is true.
...
%	where $\sigma[i-1]\sigma[i]$ appears in $\widetilde{\sigma}$
	...
	\PM^{\widetilde{\sigma}}_{\ptext}(\lb,\ub,i,j)=
	\begin{cases}
			\bigcup_{\ell>j} \PM^{\widetilde{\sigma}}_{\ptext}(\lb,\ptext[j]-1,i+1,\ell)
				& \text{if $\ppattern[i]$ is not underlined } \\
				& \text{ and $\ppattern[i]$ is not overlined} \\
			\bigcup_{\ell>j} \PM^{\widetilde{\sigma}}_{\ptext}(\lb,\ptext[j]-1,i+1,\ell)
				& \text{if $\ppattern[i]$ is not underlined } \\
				...
				& \text{ appear in $\widetilde{\sigma}$}\\
				& \text{ and $\ptext[j]=\ub$} \\
			\PM^{\widetilde{\sigma}}_{\ptext}(\lb,\ptext[j]-1,i+1,j+1)
				& \text{if $\underline{\ppattern[i]\ppattern[i+1]}$ } \\
				& \text{ appears in $\widetilde{\sigma}$}\\
				& \text{ and $\ppattern[i]$ is not overlined} \\				
			\PM^{\widetilde{\sigma}}_{\ptext}(\lb,\ptext[j]-1,i+1,j+1)
	...
	or f $\ppattern[i']$ is matched to $\ptext[j]$ then $\ppattern[i]$ must be matched to $\ptext[j]-1$.
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				$\PM^{\widetilde{\sigma}}_{\ptext}(\lb,*,i',*)$
				is solved as a base case. So $\PM^{\widetilde{\sigma}}_{\ptext}(\lb,*,i',*)$ is true only if $\sigma[i']$ is matched to the element with $\lb$.
				...
				\item $\PM^{\widetilde{\sigma}}_{\ptext}(\lb,*,i,*)$ sets
			...
			\begin{itemize}
				...
				\item  Every recursive call $\PM^{\widetilde{\sigma}}_{\ptext}(\lb,*,i',*)$ is true only if $\sigma[i']$ is matched to the element with value $\lb$. 
