...
%		it will mapped to an descent (\pi_2).
...
\begin{proof}
	The forward direction is essentially obvious.
	For the reverse direction, assume that $s_\sigma$ is a subsequence 
	of $s_\pi$ and
	let $\phi : [k] \to [n]$ be the function describing an occurrence
	of $s_\sigma$ in $s_\pi$ as a subsequence, \emph{i.e.}
	$s_\sigma[i] = s_\pi[\phi(i)]$ for $1 \leq i \leq k$.
	Define $\varphi : [k] \to [n]$ as follows:
	$\varphi(i) = \phi(i)$ for 1 $\leq i \leq k-1$ and
	$\varphi(k) = \phi(k-1) + 1$.
	We show that $\varphi$ describes an occurrence of $\sigma$ in 
	$\pi$.
	Suppose, aiming at a contradiction, that there exist
	$1 \leq i < j \leq k$ such that
	$\sigma[i] < \sigma[j]$ and
	$\pi[\varphi(i)] > \pi[\varphi(j)]$.
	Consider the following cases:

	\begin{enumerate}		
	\item if $\sigma[i] < \sigma [j]$, 
	$s_\sigma[i] = \ustep$ and $s_\pi[j] = \ustep$,
	then $s_\pi[\varphi(i)] = \ustep$ and $s_\pi[\varphi(j)] = \ustep$
	(by construction of $\varphi$).
	Since $i < j$ and $\varphi$ is an increasing function, we have
	$\varphi(i) < \varphi(j)$.
	Therefore, according to Lemma~\ref{lemma:upup},
	$\pi[\varphi(i)] < \pi[\varphi(j)]$.
